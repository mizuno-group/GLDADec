...
cdef int judge_z(int w, long[:] seed_keys) nogil:
    cdef int c, i
    c = 0
    for i in range(len(seed_keys)):
        if w == seed_keys[i]: # w is in seed_keys
            c = 1
            break
        else:
            pass
...
def _sample_topics(int[:] WS, int[:] DS, int[:] ZS, int[:, :] nzw, int[:, :] ndz, int[:] nz,
                   double[:] alpha, double[:] eta, double[:] rands, long[:] seed_keys, double seed_conf, double other_conf):
    ...
    with nogil:
        ...
        for i in range(N):
            ...
            # consider the effect of seed topics
            c = judge_z(w,seed_keys)
            # w is included in seed topics
            if c != 0:
                if rands[i] < seed_conf:
                    # remain
                    z_new = z
                else:
                    # update
                    z_new = searchsorted(dist_sum, n_topics, r)
            # not included in seed topics
            else:
                if rands[i] < other_conf:
                    # remain
                    z_new = z
                else:
                    # update
